...
import { ArrowDownToDot, ArrowUpFromDot, Boxes, Warehouse, Loader2, RefreshCw } from "lucide-react";
...
export function StockConference() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const exitsToday = pickingLogs.filter(log => {
      ...
      return pickedAt >= todayStart && pickedAt <= todayEnd;
    }).length;

    const currentStock = inventoryItems.length;
    
    const initialStock = currentStock - entriesToday + exitsToday;

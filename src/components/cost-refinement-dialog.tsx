...
export function CostRefinementDialog({ 
  isOpen, 
  onClose, 
  sales, 
  products, 
  onSave 
}: CostRefinementDialogProps) {
  ...
  const handleSaveCosts = async () => {
    ...
    try {
      ...
      toast({ 
        ...
        description: 'Ocorreu um erro ao salvar os custos.' 
      });
    } finally {
      setIsSaving(false);
    }
  ...
  const applyParsedRows = (rows: any[]) => {
      ...
      if (headers.length < 2) {
        toast({ 
          ...
          description: 'A planilha deve ter pelo menos duas colunas.' 
        });
        return;
      }

      const orderHeader = headers.find(h => 
        h.toLowerCase().includes('pedido') || h.toLowerCase().includes('order')
      );
      const costHeader = headers.find(h => 
        h.toLowerCase().includes('custo') || h.toLowerCase().includes('cost')
      );
      ...
      if (!orderHeader || !costHeader) {
        toast({ 
          ...
          description: 'A planilha deve conter uma coluna para "pedido" e uma para "custo".' 
        });
        return;
      }
      ...
      setCosts(prev => {
        ...
        rows.forEach(row => {
          ...
          if (orderCode && !isNaN(costValue)) {
            ...
            const saleToUpdate = sales.find(s => 
              normalizeOrderCode((s as any).order_code) === normalizedOrderCode
            );
            if (saleToUpdate) {
      ...
      toast({ 
        ...
        description: `${updatedCount} custos foram preenchidos a partir do arquivo.` 
      });
  };
    ...
    reader.onload = (e) => {
      try {
        ...
        toast({ 
          ...
          description: 'O formato do arquivo pode estar corrompido.' 
        });
      } finally {
        setIsParsing(false);
    ...
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="max-w-4xl h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Abaixo estão as vendas do período que não possuem um custo de produto associado. 
            Insira o custo manual para cada uma ou importe uma planilha.
          </DialogDescription>
        </DialogHeader>
                            ...
                            {paginatedSales.map(sale => {
                                ...
                                <TableRow key={sale.id}>
                                    <TableCell className="font-mono text-xs">
                                      {(sale as any).order_code}
                                    </TableCell>
                                    <TableCell>
                                      {formatDate((sale as any).payment_approved_date)}
                                    </TableCell>
                                    <TableCell className="font-medium">
                                      {friendlyName}
                                    </TableCell>
                                    <TableCell>
                                      {(sale as any).marketplace_name}
                                    </TableCell>
                                    <TableCell>
                                        <Input
                                            type="number"
        ...
        <DialogFooter className="pt-4 border-t flex-wrap-reverse sm:flex-wrap">
            ...
            <div className="flex gap-2">
              ...
              <Button 
                ...
                disabled={costs.size === 0 || isSaving}
              >
                {isSaving ? (
                  <Loader2 className="animate-spin mr-2 h-4 w-4" />
                ) : (

...
import * as Lucide from 'lucide-react'; // <— troca aqui
...
const Icons = {
  ...
  Save: Lucide.Save,
  // FilterX pode não existir -> cai pro Filter -> se não tiver, cai pro Search
  FilterX: (Lucide as any).FilterX ?? (Lucide as any).Filter ?? Lucide.Search,
  // FileSpreadsheet pode não existir -> cai pro FileText
    ...
    reader.onload = (e) => {
      try {
        ...
        if (file.name.endsWith('.csv')) {
          ...
          const workbook = XLSX.read(content as string, { type: 'binary' });
